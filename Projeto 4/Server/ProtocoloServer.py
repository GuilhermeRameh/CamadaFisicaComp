    ...
    def estadoPegandoPacotes(self):
        ...
        while cont <= int.from_bytes(pacotes_total, "big"):
            ...
            if tipo_da_mensagem==b'\x03':
                ...
                crcBuffer = binascii.crc_hqx(bufferPacote, 0).to_bytes(2,'big')
                print(f'\nO CRC do pacote que recebi É: {binascii.hexlify(crcBuffer)}\n')
                print("\nGetting EOP...")
                ...
                if bufferEOP == self.eop and int.from_bytes(id_pacote, "big") == cont and crcHead == crcBuffer:
                    ...
                    print("EOP incorreto ou ID do pacote incorreto ou CRC incorreto. \nEnviando *msgt6* para reenvio de pacote.")
                    if previousId == 0:
                        txBuffer = self.msgt2
                    else:
                        txBuffer = self.constructDatagram(b'\x06', self.id_do_sensor, self.id_do_server, pacote_recomeco=previousId)
